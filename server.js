...
// Client schema
const clientSchema = new mongoose.Schema({ firstName: String, surname: String, companyName: String, organizationNumber: String, address: String, postalNumber: String, city: String, tel: String, email: { type: String, unique: true }, membershipType: { type: String, enum: ["TaxiPro S", "TaxiPro M", "TaxiPro L", "Obegränsad"] }, paymentMethod: String, message: String, acceptTerms: Boolean, receiveNews: Boolean });
const Client = mongoose.models.Client || mongoose.model("Client", clientSchema);
...
app.post("/api/register", [
  ...
  body("city").trim().notEmpty().escape(),
  body("tel").trim().notEmpty().escape(),
  body("membershipType").isIn(["TaxiPro S","TaxiPro M","TaxiPro L","Obegränsad"]),
  body("paymentMethod").trim().notEmpty().escape(),
  body("acceptTerms").equals("true"),
], async (req, res) => {
  const errors = validationResult(req);
  ...
  try {
    const { email, firstName, surname, companyName, organizationNumber, address, postalNumber, city, tel, membershipType, paymentMethod } = req.body;

    const existingClient = await Client.findOne({ email });
    if (existingClient) return res.status(400).json({ message: "Client with this email already exists" });

    const client = new Client({ ...req.body, acceptTerms: true, receiveNews: req.body.receiveNews === true || req.body.receiveNews === "true" });
    await client.save();
    ...
    const emailStyles = `
      <style>
        @media only screen and (max-width: 600px) {
          .main { width: 100% !important; }
          .content { padding: 15px !important; font-size: 15px !important; }
          .header { font-size: 18px !important; padding: 12px !important; color: ${headerTextColor} !important; }
          .logo { max-width: 140px !important; }
        }
      </style>
    `;
    ...
    const companyEmailHTML = `...`; // unchanged for brevity

    let companyEmailSent = false;
    let customerEmailSent = false;

    try {
      const companyResponse = await axios.post(process.env.TAXIPRO_EMAIL_API, { subject: "New TaxiPro Registration", body: companyEmailHTML, emails: ["Kund@taxipro.se"] }, { headers: { "Content-Type": "application/json" } });
      if (companyResponse.status === 200) companyEmailSent = true;
    } catch (err) { console.error("Company email error:", err.message); }

    try {
      const customerResponse = await axios.post(process.env.TAXIPRO_EMAIL_API, { subject: "Welcome to TaxiPro", body: customerEmailHTML, emails: [email] }, { headers: { "Content-Type": "application/json" } });
      if (customerResponse.status === 200) customerEmailSent = true;
    } catch (err) { console.error("Customer email error:", err.message); }

    return res.status(201).json({ message: "Registration successful", companyEmailSent, customerEmailSent });
  } catch (error) {
    console.error("Registration error:", error);
    res.status(500).json({ message: "Internal server error" });
  }
